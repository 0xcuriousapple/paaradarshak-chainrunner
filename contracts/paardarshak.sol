...
contract paardarshak is owned {
    ...
    function createToken(
        ...
        donations[_key].CompleteHistoryOfToken.push(
            SingleInstanceOfHistory(_value, 0, _name, "root", owner, d)
        );
    ...
    function breakToken(
        bytes32 _key,
        uint256 _valueofparenttoken,
        bytes32 _childkey
    ) public {
        uint256 temp = donations[_key].CompleteHistoryOfToken.length - 1;

        require(
            donations[_key].CompleteHistoryOfToken[temp]._owner == msg.sender,
        ...
        uint256 valueofchildtoken = donations[_key].CompleteHistoryOfToken[temp]
            .value - _valueofparenttoken;

        donations[_key].CompleteHistoryOfToken.push(
            SingleInstanceOfHistory(
                _valueofparenttoken,
                1,
        ...
        donations[_childkey].CompleteHistoryOfToken.push(
            SingleInstanceOfHistory(
                valueofchildtoken,
                1,
                donations[_key].CompleteHistoryOfToken[temp].nameOfOwner,
    ...
    function transferToken(
        bytes32 _key,
        uint8 _typeOfOwner,
        string memory _nextOwnername,
        string memory _purpose,
        address _nextOwner
    ) public {
        require(
            donations[_key].CompleteHistoryOfToken[donations[_key]
                .CompleteHistoryOfToken
                .length - 1]
                ._owner == msg.sender,
            "You are not owner of this token"
        );
        ...
        donations[_key].CompleteHistoryOfToken.push(
            SingleInstanceOfHistory(
                ...
                _purpose,
                _nextOwner,
                donations[_key].CompleteHistoryOfToken[temp].childtoken
            )
        );
    ...
    function addAuthority(string memory _childname, address _add) public {
        require(
            AuthoritiesNames[msg.sender].status == true,
            "You are not a authority"
        );
        require(
            listedAuthorities[_add].exists,
            "The address is already registered as a Authority"
        );
    ...
    function getSingleAuthDetails(address _add) public viewreturns (authority memory){
